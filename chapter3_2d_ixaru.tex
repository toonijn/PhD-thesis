...
\end{align*}
Multiplying both sides with $\vb{b}^{(k+1)}(x)$ and integrating along the $x$-axis for each $i \in \{1, \dots, N\}$ yields
\begin{align*}
...
$$
Here we assumed the orthogonal basis $\{b_i^{(k+1)}(x)\}$ to be normalized $\langle b_{i}^{(k+1)} | b_{j}^{(k+1)} \rangle = \delta_{ij}$, or in matrix notation:
