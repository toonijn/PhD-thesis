...
The focus of this article is mainly to provide some theoretical advancements. To demonstrate the relevance and effectiveness of the provided theory, we proudly present the following numerical results. In summary the obtained results are at least as accurate as the original method as those proposed in \cite{ixaru_new_2010}, but are significantly faster, even when corrected for the more powerful modern CPUs. Following Ixaru, we take $N=12$ in all  experiments.

Another large improvement can be found in the `autonomy' of the algorithm. In \cite{ixaru_new_2010} different steps sizes are tabulated. But, since we implemented automatic step size selection, this distinction is no longer relevant. To demonstrate that our improvements facilitate using this method, we will also provide some \texttt{Python}-code. This code makes use of the package \texttt{pyslise2d}. This package is still in active development, the most recent version can be installed with \texttt{pip install pyslise2d}. The implementation of our method is done in \texttt{C++}, to improve efficiency and performance, and precompiled in \texttt{pyslise2d}.

\subsection{Ixaru's potential}\label{sec:c3_experiment_ixaru}

In the first example we look at the same problem as in \cite{ixaru_new_2010}. Consider the potential $$
  V(x, y) = (1+x^2)(1+y^2)
$$
on the square domain $[-5.5, 5.5] \times [-5.5, 5.5]$.
...
\begin{table}
  \centering
  \begin{tabular}{l|n{2}{7}n{2}{10}r}
...
\begin{table}
  \centering
  \begin{tabular}{l|rrr}
                      & Reference\cite{braun_efficient_1996} & Our results    & Difference \\
    \hline

    $E_{0}$           & $0.998595$                           & $0.9985947661$ & $2.3(-7)$  \\
    $E_{1} = E_{2}$   & $1.990077$                           & $1.9900767814$ & $2.2(-7)$  \\
    $E_{3}$           & $2.956243$                           & $2.9562429806$ & $1.9(-8)$  \\
    $E_{4} = E_{5}$   & $2.985326$                           & $2.9853263715$ & $3.7(-7)$  \\
    $E_{6} = E_{7}$   & $3.925964$                           & $3.9259638565$ & $1.4(-7)$  \\
    $E_{8}$           & $3.982417$                           & $3.9824172948$ & $2.9(-7)$  \\
    $E_{9}$           & $3.985761$                           & $3.9857610624$ & $6.2(-8)$  \\
    $E_{10}$          & $4.870144$                           & $4.8701435890$ & $4.1(-7)$  \\
    $E_{11} = E_{12}$ & $4.898644$                           & $4.8986454644$ & $1.5(-6)$  \\
    $E_{13} = E_{14}$ & $4.986251$                           & $4.9862512850$ & $2.8(-7)$  \\
    $E_{15}$          & $5.817019$                           & $5.8170190699$ & $7.0(-8)$  \\
    $E_{16}$          & $5.817027$                           & $5.8170279899$ & $9.9(-7)$  \\
  \end{tabular}
