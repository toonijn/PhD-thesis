...
\longchapter[Strands: new 2D method]{Strands: a new method for \twoD time-independent Schrödinger equations}\label{cha:c4}
...
\nlparagraph{The harmonic oscillator}
...
\nlparagraph{Finding the null space of a matrix}
...
\nlparagraph{Solving the matrix approximation}
...
\nlparagraph{Constructing a deflation space}
...
\nlparagraph{Solving the square eigenvalue problem}
...
In section~\ref{sec:c4_nm_runtime}, we will take a closer look at how both solving strategies perform.
...
The behavior we observe in figure~\ref{fig:c4_nm_zero_test_disc} is concerning. The method seems to be unable to further improve accuracy, even if parameters which should result in more accurate approximations are used. The eigenvalues found with $N=K=128$ are not consistently more accurate than the values found with $N=K=64$.
