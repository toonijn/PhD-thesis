...
\begin{figure}
    ...
    \end{center}
    \caption{These graphs display the relative error of each of the first $100$ eigenvalues of the harmonic oscillator \eqref{equ:c4_fd_harmonic}, computed for different values of $h = \eta$ and $N_x = N_y$.}
...
Going further with a theoretical analysis becomes unnecessarily difficult. We will estimate the order by using the numerical results for the first $100$ eigenvalues of the harmonic oscillator, calculated for the values of $h = \eta = \left\{\frac{19}{20}, \frac{19}{30}, \frac{19}{40}, \frac{19}{50}, \frac{19}{60}, \frac{19}{80}, \frac{19}{100}\right\}$. These $7$ different sets of parameters yield for each of the $100$ eigenvalues (with index $k \in \{1, 2, \dots, 100\}$) a data point. Next, through all these points we find the best fitting curve (in the least squares sense) which follows the formula $c_1 h^{c_2} k^{c_3}$.
...
\end{equation}
on the domain $[0, \pi] \times [0, \pi]$.

\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth]{img/chapter4/fd_zero.pdf}
    \end{center}
    \caption{These graphs display the relative error of each of the first $100$ eigenvalues of the zero potential \eqref{equ:c4_fd_zero}, computed for different values of $h = \eta$ and $N_x = N_y$.}
