...
\undefinedlabel{sec:c3_experiment_ixaru}{3.x.x}
...
$$
with $\vb{c} = \vb{V_D} \vb{v}$. Applying this method to the problem at hand yields 960 eigenvalues. First, it is important to remark that the resulting values are elements of $\CC$. But, in this case, the imaginary part of all values lies between \numprint{-1.68e-14} and \numprint{1.68e-14}, so all values may be considered real. The lowest few values are given:
$$
    \underbrace{
...
Our implementation is built in \cpp{} using \Eigen{}~\cite{guennebaud_eigen_2010} for linear algebra, \spectra{}~\cite{qiu_yixuan_2022} as eigenvalue solver, and \matslise{3} (see chapter~\ref{cha:c2} or~\cite{baeyens_fast_2020}) for the computation of one-dimensional basis functions. Installation is possible through \lpython{}'s package system \texttt{pip}.
% \begin{noindent}
\begin{minted}{bash}
pip install strands
\end{minted}
...
When comparing figure~\ref{fig:c4_nm_harmonic_rectangle} with figure~\ref{fig:c4_fd_harmonic}, we notice that our new method is able to reach the same accuracy with only a grid size of $48$ (with $48$ basis functions). This is considerably less than the required grid size of $100$ with spacing $h = 0.19$ when using the finite difference scheme. As a direct comparison, our method was solving a $\numprint{2304}\times\numprint{2304}$ sparse eigenvalue problem (from equation~\ref{equ:c4_nm_sparse_system}), while the method with finite difference approximation needed to solve a sparse $\numprint{9801}\times\numprint{9801}$ eigenvalue problem (from equation~\ref{equ:c4_fd_sparse_matrix}).
...
harmonic = Schrodinger2D(
    lambda x, y: x**2 + y**2, Rectangle(-9.5, 9.5, -9.5, 9.5),
    gridSize=(40, 40), maxBasisSize=30)
...
This program runs in under half a second. The resulting approximations agree up to all digits reported in~\cite{ixaru_new_2010}.
...
For future reference, we provide the first $26$ eigenvalues we have found with $N = 90$ and $K = 40$.
...
% Some formulae, boundary conditions, propagation, CP?
